#!/usr/bin/env python3
import sys, os, json, yaml, re, subprocess
...
def slugify(text):
    ...
    return text[:40].strip('-') or 'run'

def run_cmd(cmd, cwd=None):
    r = subprocess.run(cmd, shell=True, capture_output=True, text=True, cwd=cwd)
    return r.returncode, r.stdout.strip(), r.stderr.strip()

def get_or_create_run_dir(squad_dir, title_context="default"):
    squad_dir = Path(squad_dir).resolve()
    output_dir = squad_dir / 'output'
    output_dir.mkdir(exist_ok=True)

    today_str = datetime.now().strftime("%Y-%m-%d")
    slug = slugify(title_context)
    
    pattern = f"{today_str}_{slug}_run-*"
    existing_runs = sorted(list(output_dir.glob(pattern)))
    run_num = len(existing_runs) + 1
    
    folder_name = f"{today_str}_{slug}_run-{run_num:02d}"
    current_run_dir = output_dir / folder_name
    current_run_dir.mkdir(parents=True, exist_ok=True)

    # Symlink 'latest' -> current_run_dir
    ...
    return current_run_dir, folder_name
...
def print_squad_status(squad_dir):
    ...
    print(f"=========================================")
    print(f"• Diretório de Saídas: {squad_dir}/output/latest/")
    print(f"• Total de Passos: {len(pipe_cfg.get('steps', []))}\n")

    for step in pipe_cfg.get('steps', []):
        s_id = step['id']
        s_type = step.get('type', 'agent_execution')
        s_agent = step.get('agent', s_type)
        
        # Read execution mode from squad-party if available
        exec_mode = "subagent"
        ...
        print(f"  [{status_label}] {s_id} | Agent: {s_agent} ({exec_mode})")
    print(f"=========================================\n")
...
if __name__ == '__main__':
    if len(sys.argv) > 1:
        ...
        if cmd == 'init-run':
            ...
            title = sys.argv[3] if len(sys.argv) > 3 else "default"
            run_dir, folder_name = get_or_create_run_dir(squad_path, title)
            print(f"[Run Criada] Pasta: {folder_name}")
            print(f"[Caminho Absoluto] {run_dir}")
        elif cmd == 'status':
            squad_path = sys.argv[2]
            print_squad_status(squad_path)
    else:
        print("Uso: python3 squad-runner.py [init-run | status] /caminho/do/squad [title_context]")
